...
- 通常モードでは CPU 統計が制限される場合があります。オプションの Root モードは、まずリスクの告知を表示し、同意してから Root マネージャーに権限を要求します。失敗時は原因を表示し、アプリ内の許可ヘルプを確認できます。

### エクスポートと更新

- システムのファイル選択画面を使い、Magisk/KernelSU の ZIP モジュールを書き出します。出力内容は固定の最小セット（ブランド、モデル、デバイス名、製品名、Android バージョン/SDK、ABI 一覧）です。
- モジュールの version / versionCode はアプリのバージョンから取得します。モジュールを適用すると ro.product.* などのシステムプロパティが書き込まれ、システムの動作に影響する可能性があります。
